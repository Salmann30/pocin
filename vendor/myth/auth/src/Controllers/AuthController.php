...
class AuthController extends Controller
{
    ...
    public function login()
    {
        ...
        if ($this->auth->check()) {
            $redirectURL = session('redirect_url') ?? site_url('/profile');
    ...
    // Get the user after login
        ...
        if ($role && $role['group_id'] == 1) { // Assuming 1 is for 'superadmin'
            $redirectURL = site_url('/admin/dashboard');
        } elseif ($role && $role['group_id'] == 2) { // Assuming 2 is for 'umkm'
            $redirectURL = site_url('/umkm/produk/index');
        } else {
            $redirectURL = site_url('/'); // Fallback URL
        }
        unset($_SESSION['redirect_url']);
